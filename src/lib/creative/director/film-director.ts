...
import { resolveSafeGenerationDuration } from "./video-provider-capabilities";

// AI Director System — Film Director (2026-07-27). This module's actual
// planning logic (system prompt, schema, LLM call, duration-trimming) is
// unchanged from what previously lived directly in
// lib/creative/film-storyboard.ts — moved here as this app's first concrete
// "Director" implementation, producing a style-tagged DirectorPlan instead
// of a bespoke FilmStoryboard shape, so future callers can consume Film's
// output the same way they'd consume the Commercial Director's. Mirrors
...
// used for the final trim, not the vendor request. Exported (via
// film-storyboard.ts's compatibility re-export) for film-scene-video.ts to
// apply to its renderVideo() call.
//
// AI Director System (2026-07-27) — now DERIVED from
// video-provider-capabilities.ts's declared Seedance2 entry instead of a
// bare "4" — same value as before (Seedance is still the only real,
// currently-enabled VIDEO provider for FILM), but now traceable to one
// canonical, explicitly-vendor-labeled source instead of a magic number
// that would silently go stale the moment that assumption changed.
export const FILM_SCENE_GENERATION_FLOOR_SECONDS = resolveSafeGenerationDuration(0, "seedance2");
...
// — same defensive posture scene-split.ts establishes.
const storyboardSchema = z.object({
  scenes: z.array(storyboardSceneSchema).min(1),
});

export class FilmDirectorError extends Error {}

export interface FilmDirectorCharacterInput {
  slotIndex: number;
  name: string | null;
...
export interface FilmDirectorInput {
  ...
  characters: FilmDirectorCharacterInput[];
}

function describeCharacter(character: FilmDirectorCharacterInput): string {
...
export async function planFilm(input: FilmDirectorInput): Promise<DirectorPlan> {
  ...
  try {
    ...
    throw new FilmDirectorError("Storyboard response was not valid JSON.");
  }

  const validated = storyboardSchema.safeParse(parsed);
  if (!validated.success) {
    throw new FilmDirectorError(`Storyboard response did not match the expected shape: ${validated.error.message}`);
  }

  const scenes = trimScenesToTotalDuration(validated.data.scenes, input.totalDurationSeconds);
  return { style: "film", scenes, textOverlays: [] };
}
