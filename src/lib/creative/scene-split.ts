...
// feedback that drove film-storyboard.ts's identical rewrite (see that
...
export async function splitScriptIntoScenes(script: string, userId: string): Promise<SceneSplit> {
  ...
  try {
    ...
    throw new SceneSplitError("Scene-split response was not valid JSON.");
  }

  const validated = sceneSplitSchema.safeParse(parsed);
  if (!validated.success) {
    throw new SceneSplitError(`Scene-split response did not match the expected shape: ${validated.error.message}`);
  }
