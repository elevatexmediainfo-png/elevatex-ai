...
export function computeBrollTargetRange(sourceDurationMs: number, density: "MINIMAL" | "BALANCED" | "HEAVY" | undefined): { min: number; max: number } {
  ...
  const min = Math.max(1, Math.round(perMinMin * minutes));
  const max = Math.max(min, Math.round(perMinMax * minutes));
  return { min, max };
}
...
Vary the intensity by how strong the moment is — don't use the same scaleTo every time: a mild emphasis might be 105-110%, a stronger one 115%, a genuine peak moment up to 120% (never above 150 regardless). Keep scaleFrom around 100 (native size). AVOID REPETITIVE RHYTHM — do not zoom on a fixed interval or use the identical scale value for every window; if you're proposing several zooms across the video, vary both their spacing and their intensity so it doesn't read as a metronome. Propose 0 to 5 zoom windows total depending on how many of the above genuinely occur — it's fine to propose zero for a video with no real emphasis moments, questions, or numbers.

TASK 3 — broll: ${brollDensityGuidance}${stockOnlyGuidance} Only propose a slot where the transcript mentions something concrete and visualizable that would genuinely benefit from a supporting shot (a specific object, place, action, or scene) — never for abstract statements, filler, or when you're not confident there's a real visual to show.
...
- sfx: propose 0-4 short one-shot sound effects tied to genuine TIMELINE EVENTS, not sprinkled arbitrarily — a whoosh or swipe at a b-roll cut or transition, a pop or click at a caption's punchy entrance or a sticker appearing, an impact or text-pop at the video's single strongest emphasis moment, a sparkle at a positive/success beat. Reuse the video-understanding emphasis moments given above as your primary cue for WHERE when available. NEVER SPAM — most videos genuinely warrant 1-3 of these at most; only propose more when there are truly that many distinct, real cut/emphasis events to accent. Set "assetQuery" to a short literal sound description (e.g. "whoosh", "swipe", "click", "pop", "impact", "sparkle", "transition", "text pop") and "atMs" to the specific instant. Never set assetId, resolvedAssetUrl, or resolutionNote.
