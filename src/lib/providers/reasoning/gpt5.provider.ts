...
- LANGUAGE REGISTER: if the transcript is spoken in Hindi or Hinglish, caption TEXT MUST use Roman/Latin script — NEVER Devanagari script, no exceptions, regardless of style preset. It must ALSO never read as a stiff, academic/literal transliteration (spelling out every vowel sound the way a textbook romanization system would) — write it exactly the way a real Indian creator types their own reel captions: natural spelling, numerals where a real caption would actually use them, contractions and colloquialisms intact. Example, all three for the SAME spoken line: "जिससे हमें चार साल की उम्र में" is WRONG (Devanagari script); "Jisase hamem caar saal kii umar mem" is ALSO WRONG (technically sounds-out-correct, but stiff academic transliteration no real creator would type); "Jisse humein 4 saal ki umar mein" is CORRECT (natural, colloquial, the numeral used naturally, exactly how a real Hinglish caption reads). Mix in English naturally where a real bilingual creator would ("Sugar Control", "Morning Routine", "Health Tip", "Business Mistake") rather than translating everything into one language mechanically. If the transcript is entirely in English, this doesn't apply — keep captions in English.
...
POWER-WORD HIGHLIGHTING — captions should look like a premium, modern Instagram Reels / YouTube Shorts caption: bold, clean, highly readable, mobile-first, attention-grabbing — never a wall of plain, same-colored text, and never a rainbow either. For a caption whose wording genuinely has a standout word or two, set "highlightWords" to color ONLY those words, using EXACTLY these THREE colors and no others: white (#FFFFFF, PRIMARY — every normal word; the default, you don't need to set this explicitly, and most words in any caption should stay this color), yellow (#FFD60A, ACCENT 1 — reserve this for the SINGLE strongest word in the caption, per the priority order below), cyan/light blue (#5AC8FA, ACCENT 2 — a genuine secondary emphasis word, only when a real second one exists). USUALLY JUST 1-2 highlighted words per caption — that is the normal case, not the exception; only rarely, when a caption's wording genuinely has that many distinct standout moments, does a third or fourth ever belong. Not every caption needs highlighting at all — a plain, all-white caption is a completely valid, common choice. Do NOT use red, green, or any other color — only these three. Do NOT color most of the caption — the large majority of words stay the default white/primary color. Do NOT randomly alternate colors word-by-word with no semantic reason. Do NOT color every word.

WHICH WORD(S) TO HIGHLIGHT — when deciding what genuinely deserves ACCENT 1 (yellow) or ACCENT 2 (cyan), prioritize in this order (a single caption rarely has all of these — pick whichever genuinely apply to THIS caption's own wording, never force a category that isn't actually there):
1. Numbers (a count, a step, a list size — "3", "TOP 5", "STEP 2").
2. Money / prices / percentages ("₹10,000", "50% off", "DOUBLE").
3. Business keywords (the core business term the moment is actually about — "BUSINESS", "SALES", "PROFIT", "GROWTH").
4. Action words (what the viewer should DO — "START", "GROW", "STOP", "SAVE", "AVOID").
5. Strong emotional words (the word carrying this moment's emotional weight — "MISTAKE", "SECRET", "WARNING", "HURT").
6. Contrast words (a turn/negation/before-after word — "BUT", "NEVER", "INSTEAD", "NAHI").
7. Important nouns (the specific concrete thing this caption is actually about).
8. CTA words (a call-to-action word — "FOLLOW", "SAVE", "COMMENT", "SHARE").
Give the single strongest word from this priority order yellow (ACCENT 1); give the next-strongest word (only if a genuinely distinct second one exists — don't invent one) cyan (ACCENT 2). This stays GPT's own judgment call every time — never a fixed keyword list — but every colored word must trace back to a real reason from this priority order, not a decorative or random pick. Example: for the caption "Apna BUSINESS grow karna hai?", you might set highlightWords to [{"word":"BUSINESS","color":"#FFD60A"},{"word":"grow","color":"#5AC8FA"}] (business keyword outranks the action word here), leaving every other word the default white. For "Sales DOUBLE karni hai?", you might set [{"word":"DOUBLE","color":"#FFD60A"},{"word":"Sales","color":"#5AC8FA"}] (a money/percentage-class word outranks the business keyword here). Each "word" must be spelled EXACTLY as it appears in this caption's own "text" (case-insensitive match is fine, but the letters must match). For the overall caption look, prefer a strong, bold, modern Latin font already available in this app's own font picker — "Poppins" or "Montserrat" are good defaults — set "style.fontFamily" explicitly, with "style.fontWeight" 700-900, unless a style preset calls for something else.
