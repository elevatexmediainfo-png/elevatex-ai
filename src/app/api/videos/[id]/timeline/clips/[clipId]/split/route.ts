...
import { splitClipSchema } from "@/lib/validations/editor";
import { InvalidStateError, splitClip } from "@/lib/timeline/engine";
import { recordProjectVersion } from "@/lib/projects/versioning";

// POST /api/videos/[id]/timeline/clips/[clipId]/split — Timeline Editor's
// "split clip" at an offset (ms) measured from the clip's own start.
export async function POST(
  ...
  try {
    ...
    const data = splitClipSchema.parse(body);
    ...
    const result = await splitClip(id, clipId, data.offsetMs);
    ...
    console.error("POST /api/videos/[id]/timeline/clips/[clipId]/split failed", err);
