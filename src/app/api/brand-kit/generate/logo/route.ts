...
import { generateBrandLogoSchema } from "@/lib/validations/brand-kit";
import { generateBrandLogo } from "@/lib/brand-kit/identity-engine";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const data = generateBrandLogoSchema.parse(body);
    const brandKit = await generateBrandLogo(session.user.id, data);
    ...
    console.error("POST /api/brand-kit/generate/logo failed", err);
    return apiError("ERR_INTERNAL", "Something went wrong generating your logo.", 500);
  }
}
