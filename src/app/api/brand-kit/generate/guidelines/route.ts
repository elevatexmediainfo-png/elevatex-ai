...
import { generateBrandGuidelinesSchema } from "@/lib/validations/brand-kit";
import { generateBrandGuidelines } from "@/lib/brand-kit/identity-engine";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const data = generateBrandGuidelinesSchema.parse(body);
    const brandKit = await generateBrandGuidelines(session.user.id, data);
    ...
    console.error("POST /api/brand-kit/generate/guidelines failed", err);
    return apiError("ERR_INTERNAL", "Something went wrong generating your guidelines.", 500);
  }
}
