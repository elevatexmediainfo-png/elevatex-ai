...
import { ungroupEditorClipsSchema } from "@/lib/validations/video-editor";
import { ungroupClips } from "@/lib/video-editor/clips";
import { InvalidStateError } from "@/lib/video-editor/errors";

// POST /api/editor/projects/[id]/clips/ungroup — Module 2's Ungroup.
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const body = ungroupEditorClipsSchema.parse(await req.json());
    ...
    const clips = await ungroupClips(id, body.clipIds);
    ...
    console.error("POST /api/editor/projects/[id]/clips/ungroup failed", err);
